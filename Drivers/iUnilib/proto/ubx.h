...
#define UBX_MSG_NAV_PVT                         ((uint8_t)0x07)
...
{
    uint32_t week_msseconds;                        // Количество милисекунд с начала недели. Неделя начинается в воскресенье
    uint16_t year;                                  // Год в формате UTC
    uint8_t month;                                  // Месяц UTC (1..12)
    uint8_t day;                                    // День месяца UTC (1..31)
    uint8_t hour;                                   // Час UTC (0..23)
    uint8_t min;                                    // Минуты UTC (0..59)
    uint8_t sec;                                    // Секунды UTC (0..59)
    uint8_t dataValid:1;                            // Валидность даты
    uint8_t timeValid:1;                            // Валидность времени
    uint8_t dateTimeResolved:1;                     // Полностью ли разпознано время
    uint8_t magValid:1;                             // Валидность магнитного склонения
    uint32_t timeAcc;                               // Точность времени, нс
    int32_t secFract;                               // Дробная часть секунд, нс
    uint8_t fixType;                                // Тип захвата GPS
    uint8_t flags_gnssFixOk:1;                      // Есть захват GPS
    uint8_t flags_diffSolution:1;                   // В решении используются дифференциальные данные
    uint8_t flags_psm_state:3;                      // Текущий режим сохранения энергии
    uint8_t flags_head_of_vehicle_valid:1;          // Курс машины (несущей GPS модуль) валидный
    uint8_t flags_carrier_solution:2;               // Текущий статус несущей решения
    uint8_t flags2_reserved;
    uint8_t numSv;                                  // Количество спутников в решении
    int32_t longitude;                              // Долгота, 1E-7 град
    int32_t lattitude;                              // Широта, 1E-7 град
    int32_t height;                                 // Высота над поверхностью эллипсоида, мм
    int32_t hMSL;                                   // Высота над уровнем моря, мм
    uint32_t hAcc;                                  // Точность позиционирования по горизонтали, мм
    uint32_t vAcc;                                  // Точность позиционирования по вертикали, мм
    int32_t velN;                                   // Скорость в проекции на север, мм/сек
    int32_t velE;                                   // Скорость в проекции на восток, мм/сек
    int32_t velD;                                   // Скорость в проекции вниз, мм/сек
    int32_t gSpeed;                                 // Скорость в проекции на землю, мм/сек
    int32_t heading;                                // Направление движения, 1E-5 град
    uint32_t sAcc;                                  // Точность определения скорости, мм/сек
    uint32_t headAcc;                               // Точность определения направления, 1E-5 град
    uint16_t pDOP;                                  // Position DOP, 0.01
    uint8_t reserved[6];
    int32_t headVehicle;                            // Направление движения машины (несущей ГПС модуль), 1E-5 град
    int16_t magDec;                                 // Магнитное склонение, 1E-2 град
    uint16_t magDecAcc;                             // Точность определения магнитного склонения, 1E-2 град
} ubx_nav_pvt_t;
