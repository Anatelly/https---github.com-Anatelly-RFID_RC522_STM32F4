...
void sx1276_lora_device_init(sx1276lora_t *trc, struct termios *opt)
{
    ...
    // Занулим изначально функции переключения тракта
    trc->transc.ll_func.inAir = 0;
    trc->transc.ll_func.switch_to_rx = NULL;
    trc->transc.ll_func.switch_to_tx = NULL;
...
static void sx1276_lora_poll (sx1276lora_t *trc)
{
    ...
    if (qty)
    {
        // Если есть данные и мы еще не вышли на передачу - переключаем тракт
        if (!(trc->transc.ll_func.inAir))
        {
            trc->transc.ll_func.inAir = 1;
            if (trc->transc.ll_func.switch_to_tx != NULL) trc->transc.ll_func.switch_to_tx();
        }

        // Если трансивер не занят, то отправляем следующую пачку данных
        if (!trc->transc.busy)
        {
            // Сначала посмотрим, если мы только выходим на передачу - надо переключить тракт (если функция переключения задана)
    ...
    else
    {
        ...
        if (!trc->transc.busy)
        {
            // Трансивер больше не занят - можно переключаться на прием
            trc->transc.ll_func.inAir = 0;
            if (trc->transc.ll_func.switch_to_rx != NULL) trc->transc.ll_func.switch_to_rx();
        }
    }
...
{
	return sx1276_lora_write(&sx1276_lora1, buffer, len);
}

ssize_t sx1276_lora1_read (char *buffer, size_t len)
{
	return sx1276_lora_read(&sx1276_lora1, buffer, len);
}
...
{
	return sx1276_lora_write(&sx1276_lora2, buffer, len);
}

ssize_t sx1276_lora2_read (char *buffer, size_t len)
{
	return sx1276_lora_read(&sx1276_lora2, buffer, len);
}
...
{
	return sx1276_lora_write(&sx1276_lora3, buffer, len);
}

ssize_t sx1276_lora3_read (char *buffer, size_t len)
{
	return sx1276_lora_read(&sx1276_lora3, buffer, len);
}
