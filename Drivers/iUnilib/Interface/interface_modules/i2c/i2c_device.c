...
{
	if(i2c_read_block(&i2c1, buffer, len))
		return len;
...
void i2c_device_init(i2c_t* const i2c, struct termios *opt)
{
...
#if F3_CHECK
	i2c->handler->Init.Timing = opt->c_cc[V_I2C_TIMING];

	i2c->handler->Init.OwnAddress2Masks = opt->c_cc[V_I2C_OwnAddress2Masks];
#elif F1_CHECK
	// Clock
	...
	if(opt->c_cc[V_I2C_OWNADRESS_2])
		i2c->handler->Init.OwnAddress2 = opt->c_cc[V_I2C_OWNADRESS_2];
